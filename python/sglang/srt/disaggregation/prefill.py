...
class SchedulerDisaggregationPrefillMixin:
    ...
    def process_batch_result_disagg_prefill(
        ...
        if self.attn_tp_rank == 0:
            ...
            self.last_input_throughput_schedule_time = batch.num_prefill_tokens / (
                process_result_time - batch.schedule_batch_time
            )
            self.last_input_throughput_run_time = batch.num_prefill_tokens / (
                process_result_time - batch.run_batch_time
            )
            logger.info(
                f"Prefill batch. "
            ...
            if self.enable_metrics:
                self.stats.input_throughput_schedule_time = (
                    self.last_input_throughput_schedule_time
                )
                self.stats.input_throughput_run_time = (
                    self.last_input_throughput_run_time
                )
