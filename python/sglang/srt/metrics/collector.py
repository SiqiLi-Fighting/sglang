...
class SchedulerMetricsCollector:
    ...
    def gather_stats(
        ...
        global_info = torch.empty((dp_size, attn_tp_size, 15), dtype=torch.float32)
    ...
    def log_stats(
        ...
        stats_list = self.gather_stats(
            stats, dp_size, attn_tp_rank, attn_tp_size, tp_cpu_group
        )
        if attn_tp_rank != 0:
            return
        ...
        for i, stat in enumerate(stats_list):
            ...
            self._log_gauge_with_dp(
                self.num_grammar_queue_reqs, stat.num_grammar_queue_reqs, i
            )
            ...
            self._log_gauge_with_dp(
                self.avg_request_queue_latency, stat.avg_request_queue_latency, i
            )
            self._log_gauge_with_dp(
                self.input_throughput_schedule_time,
                stat.input_throughput_schedule_time,
                i,
            )
            self._log_gauge_with_dp(
                self.input_throughput_run_time, stat.input_throughput_run_time, i
            )
            # Disaggregation queue metrics
            self._log_gauge_with_dp(
                self.num_prefill_prealloc_queue_reqs,
                stat.num_prefill_prealloc_queue_reqs,
                i,
            )
            self._log_gauge_with_dp(
                self.num_prefill_infight_queue_reqs,
                stat.num_prefill_infight_queue_reqs,
                i,
            )
            self._log_gauge_with_dp(
                self.num_decode_prealloc_queue_reqs,
                stat.num_decode_prealloc_queue_reqs,
                i,
            )
            self._log_gauge_with_dp(
                self.num_decode_transfer_queue_reqs,
                stat.num_decode_transfer_queue_reqs,
                i,
            )
